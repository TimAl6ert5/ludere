...
PATTERNS_FROM = [
    (r'[\.-]+', TOKEN_MC_TEXT),
    (r' {7}', TOKEN_MC_WORD), # order matters, look for the longer word, before the shorter space sequence
    (r' {3}', TOKEN_MC_SPACE)
]


class Token:
    def __init__(self, tkn_type: int, value: str, begin: int=None, end: int=None) -> None:
        self.type = tkn_type
        self.value = value
        self.begin = begin
        self.end = end
    ...
    def __repr__(self) -> str:
        return "<Token type({}) value({}) begin({}), end({})>".format(self.type, self.value, self.begin, self.end)
    
    def __eq__(self, __value: object) -> bool:
        if not isinstance(__value, Token):
            return False
        else:
            return self.type == __value.type and self.value == __value.value and self.begin == __value.begin and self.end == __value.end
...
class UnhandledCharacterException(Exception):
    ...
    def __str__(self) -> str:
         return "Unhandled character {} at index {}.".format(self.character, self.position)
...
class Lexer:
    ...
    def tokenize(self):
        ...
        while self.cursor.index < len(self.text):
            ...
            if not match:
                if self.strict_mode:
                    raise UnhandledCharacterException(self.text[self.cursor.index], self.cursor)
                else:
                    ...
                    if last_token is not None and last_token.type == TOKEN_UNSUPPORTED:
                        last_token.value = last_token.value + self.text[self.cursor.index]
                        last_token.end = self.cursor.index + 1
                    else:
                        token = Token(TOKEN_UNSUPPORTED, self.text[self.cursor.index], self.cursor.index, self.cursor.index + 1)
                        tokens.append(token)
                    self.cursor.advance()
            else:
